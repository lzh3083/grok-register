...
提取接口把业务错误放在 `msg` 字段里返回，HTTP 状态码仍是 200。
常见两类错误含义完全不同，别混淆：

| 返回 | 含义 | 处理 |
|---|---|---|
| `xxx not added to whitelist` | 白名单未通过 | 把返回里的 `request_ip` 加入后台白名单 |
| `流量已过期` | **套餐状态问题** | 到后台「我的套餐/订单」检查有效期 |

**关于「流量已过期」**：它指的是**套餐有效期**，与剩余流量多少无关。
可能出现「可用流量 0.92GB」但仍报过期的情况 —— 那是套餐到期后残留的流量。
后台「长度：`--`」通常意味着有效期未设置或已失效。

排查顺序（从外到内，逐层排除）：

```bash
# 1. 纯 curl 直连，排除代码因素
curl "http://api.lajiaohttp.com/api/extract_ip?regions=US&num=1&protocol=http&type=txt&cate=2&t=10&lb=1"

# 2. 换一个已在白名单的源 IP，排除源 IP 因素
curl --socks5-hostname 127.0.0.1:1080 "http://api.lajiaohttp.com/api/extract_ip?..."

# 3. 若两者返回相同错误 → 确定是账户侧问题，不是配置问题
```

已确认与下列因素**无关**（实测均返回同一错误）：
源 IP 是否在白名单内、地区代码写法（`US`/`1`/`us`/`美国`）、
会话类型（`cate=1`/`cate=2`）、协议（`http`/`https`）、
数据格式（`txt`/`json`）、是否带 `key` 参数。
...
`ready_check.sh` 会**真实启动一次浏览器并断言 `platform=Win32`** ——
...
| 节点会成批失效 | 生成后数十分钟内可能大面积掉线（实测 13 个里仅剩 2 个可用，报 `connect proxy error`）。**跑批前先点面板「测试节点」**，别按生成数量估算可用量 |
...
> **注意「可用」的口径**：面板只把 `probe_status=healthy` 的节点算作健康。
> 从未探测过的节点会显示为「未探测」，不会被算成可用 —— 早期版本按
> `enabled` 统计，会把已失效但尚未标记的节点显示成「可用」，容易误判。
...
### 坑 0：住宅代理节点会动态掉线（影响成功率）

实测节点**生成时可用，几分钟后就失效**。跑批 5 个账号时观察到两种表现：

| 表现 | 后果 |
|---|---|
| 注册中失效 | 页面状态卡住：反复点「Sign up with email」但输入框不出现（`inputs=none`） |
| CPA 导出时失效 | 注册已成功但凭据没导出，报 `Tunnel connection failed: 502` |

**这是住宅代理的固有特性，不是配置错误。** 应对方式：

1. **多备节点**：需要 N 个账号就生成 N×2 个，逐个探测后取可用的
2. 生成流程已自动剔除探测失败的节点
3. 注册过程中失效的节点，代理池会**自动冷却**并换节点重试
4. CPA 导出失败时**账号仍然保留**（注册成果不丢），可事后补导出

> 结论：**账号成功率会低于 100%，这是代理特性决定的**，不要误判为代码问题。
> 实测 5 个账号成功 4 个（80%），而成功导出的凭据 **4/4 全部真实可用**。
